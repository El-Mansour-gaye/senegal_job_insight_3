    ...
    <header className="w-full flex flex-col items-center pt-8 pb-4 px-6 z-50">
      ...
      <div className="text-center mb-8">
        ...
        <motion.p
          ...
          className="text-slate-400 mt-2 text-sm md:text-base font-medium max-w-2xl mx-auto"
        >
      ...
      {isDashboard && (
        <motion.div
          ...
          className="flex flex-wrap justify-center items-center gap-3 p-1.5 bg-slate-900/40 backdrop-blur-md border border-slate-800/50 rounded-2xl"
        >
          <button
            onClick={() => setPersona('analyst')}
            className={cn(
              "flex items-center gap-2 px-4 py-2 rounded-xl transition-all text-xs font-bold",
              persona === 'analyst' ? "bg-white text-slate-900" : "text-slate-400 hover:text-white"
            )}
          >
            <BarChart3 size={14} /> Décideur
          </button>
          <button
            onClick={() => setPersona('candidate')}
            className={cn(
              "flex items-center gap-2 px-4 py-2 rounded-xl transition-all text-xs font-bold",
              persona === 'candidate' ? "bg-white text-slate-900" : "text-slate-400 hover:text-white"
            )}
          >
            <Users size={14} /> Candidat
          </button>
          <div className="w-px h-4 bg-slate-800 mx-1 hidden sm:block" />
          <button
            onClick={handlePowerBIDownload}
            className="flex items-center gap-2 px-4 py-2 rounded-xl text-slate-400 hover:text-white transition-all text-xs font-bold"
          >
            <FileBarChart size={14} /> Rapport PowerBI
          </button>
          <button
